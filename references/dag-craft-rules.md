...
- Deterministic checks (DAG-01/02/04/05/06/07 + version stamp) run via `node ${CLAUDE_PLUGIN_ROOT}/scripts/craft-check.js [chapter-path]` at editor Pass 1 start.
...
- Auto-revise rules: DAG-01 (story opener), DAG-02 (density underflow), DAG-05 (overflow), DAG-06 (hedging, transliteration overflow), DAG-08 (fabricated testimony).
